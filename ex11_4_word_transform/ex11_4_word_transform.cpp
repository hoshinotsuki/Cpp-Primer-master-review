...
map<string, string> buildMap(ifstream &map_file)
{
	...
	while (map_file >> key && getline(map_file, value))
		if (value.size() > 1) // check that there is a transformation
			trans_map[key] = value.substr(1); //由于getline每次接受的都是上一个的终止符，所以去掉空格
		else
			throw runtime_error("no rule for " + key);//如果key没有value，throw 异常
	return trans_map;
}
...
{
	auto trans_map = buildMap(map_file); // 构建映射表 map<string, string>

										 // for debugging purposes print the map after its built
	cout << "Here is our transformation map: \n\n";
	for (auto &entry : trans_map)//取地址
